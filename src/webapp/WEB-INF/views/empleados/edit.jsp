...
<div class="container">
<a href="${pageContext.request.contextPath}/empleados/list" type="button" class="btn btn-outline-dark btn-sm" data-toggle="tooltip" data-placement="top" title="Regresar al Listado"><i class="bi-arrow-bar-left"></i></a>
<h2>Editar Empleado: <span style="color:red;"> ${employee.id} </span> </h2>
<form:form action="keep" modelAttribute="employee" method="POST">
...
<div class="row">
	<div class="col-md-6 mb-3">
		...
		<c:forEach var="ItemTemp" items="${listado}">
        	<form:option value="${ItemTemp.id}" label="${ItemTemp.description}"/>
        </c:forEach>  
        </form:select>  
	</div>
</div>
<div class="row">
	<div class="col-md-6 mb-3">
		...
		<c:forEach var="ItemTemp" items="${usuarios}">
        	<form:option value="${ItemTemp.username}" label="${ItemTemp.username}"/>
        </c:forEach>  
        </form:select>  
	</div>
</div>
